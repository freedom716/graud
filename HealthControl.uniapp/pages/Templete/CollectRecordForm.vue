<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="收藏记录信息" />

        <uni-section title="请填写收藏记录信息">
            <view class="form-container">

                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    label-position='top'>
                    <uni-forms-item label="收藏人" required name="CollectUserId">
                        <SigleSelect url="/User/List" columnName="Name" columnValue="Id" v-model="formData.CollectUserId">
                        </SigleSelect>
                    </uni-forms-item>
                    <uni-forms-item label="关联资源Id" required name="RelativeId">
                        <uni-number-box :min="0" :max="99999" :value="formData.RelativeId" />
                    </uni-forms-item>
                    <uni-forms-item label="收藏类型" required name="CollectType">
                        <uni-easyinput v-model="formData.CollectType" placeholder="请输入收藏类型" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                </uni-forms>
...
const createOrEditAsync = async () => {
    editModalForm.value.validate().then(res => {
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    let { Data, Success } = await Post("/CollectRecord/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
