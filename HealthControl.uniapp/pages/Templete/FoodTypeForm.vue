<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="食物类型信息" />

        <uni-section title="请填写食物类型信息">
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/FoodType/Get`, { Id: Id });
...
const createOrEditAsync = async () => {
    editModalForm.value.validate().then(res => {
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    let { Data, Success } = await Post("/FoodType/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
