<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="运动记录信息" />

        <uni-section title="请填写运动记录信息">
            <view class="form-container">
                ...
                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    ...
                    <uni-forms-item label="运动单位" required name="SportUnitId">
                    ...
                    <uni-forms-item label="记录值" required name="RecordValue">
                        <uni-number-box :min="0" :max="99999" :value="formData.RecordValue" />
                    </uni-forms-item>
...
const createOrEditAsync = async () => {
    editModalForm.value.validate().then(res => {
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    let { Data, Success } = await Post("/SportRecord/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
