<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="健康知识分类信息" />

        <uni-section title="请填写健康知识分类信息">
            <view class="form-container">
                ...
                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    ...
                    <uni-forms-item label="显示顺序" required name="Sort">
                        <uni-number-box :min="0" :max="99999" :value="formData.Sort" />
                    </uni-forms-item>
                </uni-forms>
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/HealthArticleType/Get`, { Id: Id });
...
const createOrEditAsync = async () => {
    editModalForm.value.validate().then(res => {
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    let { Data, Success } = await Post("/HealthArticleType/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
